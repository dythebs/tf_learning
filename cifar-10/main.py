...
#第一层卷积
W_conv1 = weight_varialbe([5,5,3,32],1e-4)
b_conv1 = bias_variable([32])
h_conv1 = tf.nn.relu(conv2d(x_image,W_conv1) + b_conv1)
h_pool1 = max_pool_3x3(h_conv1)
#第二层卷积
W_conv2 = weight_varialbe([5,5,32,32],1e-2)
...
h_pool3 = avg_pool_3x3(h_conv3)
#全连接层
W_fc1 = weight_varialbe([4*4*64,64],0.1)
...
#输出层
W_fc2 = weight_varialbe([64,10],0.1)
b_fc2 = bias_variable([10])
y = tf.nn.softmax(tf.matmul(h_fc1_drop,W_fc2) + b_fc2)
y_ = tf.placeholder(dtype=tf.float32,shape=[None,10])

cross_entropy = -tf.reduce_mean(y_*tf.log(tf.clip_by_value(y,1e-8,1)))
train_step = tf.train.AdamOptimizer(1e-4*3).minimize(cross_entropy)
#评估模型
